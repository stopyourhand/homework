...
public class CourseFolderController {
    ...
    public Result createFolder(@RequestParam("courseInfoId") int courseInfoId,
                               @RequestParam("folderName") String courseFolderName,
                               @RequestParam("courseClass") List<String> courseClass){
        System.out.println("courseFolderName" + courseFolderName);
        List<String> repeatCourseClassList = courseFolderService.createFolder(courseInfoId, courseFolderName, courseClass);
        ...
        if(!repeatCourseClassList.isEmpty()){
            ...
            return new Result(201,"班级"+repeatCourseClassString+"已经在文件夹"+courseFolderName+"中存在，故班级"+repeatCourseClassString+"创建失败!");
        }
